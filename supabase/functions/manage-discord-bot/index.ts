...
const MODULES: Record<string, { label: string; premium: boolean; title: string; description: string; color: number; buttons: any[] }> = {
  ...
  departments: { label: 'Anunțuri angajați', premium: true, title: '📢 Anunțuri · Angajați', description: 'Publică anunțuri, întrebări, sondaje, avertismente și sancțiuni pentru angajați.', color: 0x8b5cf6, buttons: [{ label: 'Publică anunț', style: 1, id: 'panel:announcements:departments:create:announcement' }, { label: 'Pune întrebare', style: 2, id: 'panel:announcements:departments:create:question' }, { label: 'Creează sondaj', style: 3, id: 'panel:announcements:departments:create:poll' }, { label: 'Avertisment', style: 4, id: 'panel:announcements:departments:create:warning' }, { label: 'Sancțiune', style: 4, id: 'panel:announcements:departments:create:sanction' }, { label: 'Istoric avertismente / sancțiuni', style: 2, id: 'panel:discipline:departments:history' }] },
  contracts: { label: 'Contracte', premium: true, title: '📄 Contracte · Panel Pro', description: 'Generează și trimite contracte folosind șablonul organizației.', color: 0x14b8a6, buttons: [{ label: 'Creează contract', style: 1, id: 'panel:contracts:create' }, { label: 'Informații necesare', style: 2, id: 'panel:contracts:info' }, { label: 'Setează contractul', style: 2, id: 'panel:contracts:settings' }, { label: 'Setează adresa', style: 2, id: 'panel:contracts:address' }] },
...
function autoRouteChannels(channelList: any[], guildId: string, currentRoutes: Record<string, any> = {}, definitions: Record<string, any> = MODULES) {
  ...
  const matchers: Record<string, string[]> = {
    ...
    actions_organization: ['actiuni organizatie', 'actiuni'], weekly_reports: ['raport saptamanal pontaj', 'raport pontaj'], marketplace: ['marketplace'], illegal_marketplace: ['marketplace ilegal'], event_reminders: ['evenimente', 'remindere'],
  ...
  const logTerms: Record<string, string[]> = { log_pontaj: ['pontaj'], log_requests_organization: ['invoiri organizatie'], log_requests_departments: ['invoiri angajati'], log_announcements_organization: ['anunturi organizatie'], log_announcements_departments: ['anunturi angajati'], log_contracts: ['contracte'], log_contract_identity_weekly: ['raport saptamanal contracte'], log_actions_organization: ['log actiuni organizatie', 'actiuni organizatie'], log_marketplace: ['marketplace'], log_illegal_marketplace: ['marketplace ilegal'], log_stash: ['stash'], log_stash_requests: ['cereri stash'], log_stash_donations: ['donatii stash'], log_event_reminders: ['evenimente', 'remindere'] };
  for (const logKey of Object.values(LOG_ROUTES)) {
    const terms = logTerms[logKey] || []; const found = rows.find((row: any) => /(^| )(log|audit)( |$)/.test(row.search) && !(logKey === 'log_marketplace' && row.nameSearch.includes('ilegal')) && !(logKey === 'log_illegal_marketplace' && !row.nameSearch.includes('ilegal')) && terms.some((term) => row.search.includes(normalize(term)))); const channelId = found?.id || logFallback;
...
async function provisionOfficialServer(db: any, guildId: string) {
  ...
  let existing = await api('/channels'); const channelIds: Record<string,string> = {}; const obsoleteNames = new Set(['menu-principal-discord','menu-configurare-discord','ajutor-panel-pro']);
  for (const obsolete of (Array.isArray(existing) ? existing : []).filter((channel: any) => Number(channel.type) === 0 && obsoleteNames.has(String(channel.name || '').toLowerCase()))) { await fetch(`${DISCORD_API}/channels/${obsolete.id}`, { method: 'DELETE', headers }).catch(() => null); existing = existing.filter((channel: any) => String(channel.id) !== String(obsolete.id)); }
  const categories = ['📌 START AICI','📚 DOCUMENTAȚIE','🛠️ SUPORT','🌐 COMUNITATE','💎 PREMIUM','📊 INFORMAȚII BOT','🔒 STAFF']; const categoryIds: Record<string,string> = {};
  ...
  for (const cat of Object.keys(groups)) for (const name of groups[cat]) { const aliases = name === '📜・reguli' ? ['📜・reguli','rules'] : name === '🧩・module-disponibile' ? ['🧩・module-disponibile','module-disponibile','module disponibile'] : [name]; const existingChannel=(Array.isArray(existing)?existing:[]).find((c:any)=>Number(c.type)===0&&aliases.includes(String(c.name))); const overwrites:any[]=[]; if(readOnly.has(name)) overwrites.push({id:guildId,type:0,allow:'1024',deny:'2048'}); if(botUserId) overwrites.push({id:botUserId,type:1,allow:'68608'}); if(cat==='🔒 STAFF') { overwrites.push({id:guildId,type:0,allow:'0',deny:'1024'}); overwrites.push({id:roleIds.Staff,type:0,allow:'68608'}); overwrites.push({id:roleIds.Support,type:0,allow:'68608'}); } if(cat==='💎 PREMIUM' && name!=='💎・preturi-si-premium') { overwrites.push({id:guildId,type:0,allow:'0',deny:'1024'}); overwrites.push({id:roleIds.Premium,type:0,allow:'68608'}); } if(existingChannel){channelIds[name]=String(existingChannel.id); if(name==='📜・reguli') channelIds.rules=String(existingChannel.id); if(name==='🧩・module-disponibile') { channelIds['module-disponibile']=String(existingChannel.id); channelIds['module disponibile']=String(existingChannel.id); } if(readOnly.has(name) || cat==='🔒 STAFF' || (cat==='💎 PREMIUM' && name!=='💎・preturi-si-premium')) await fetch(`${DISCORD_API}/channels/${existingChannel.id}`,{method:'PATCH',headers,body:JSON.stringify({permission_overwrites:overwrites})}); continue;} const made=await api('/channels',{name,type:0,parent_id:categoryIds[cat],permission_overwrites:overwrites}); channelIds[name]=String(made.id); created++; }
   ...
   const messages: Record<string, any> = {
    ...
    '💎・preturi-si-premium': { embeds: [{ title: '💎 Panel Pro · prețuri și activare', description: 'Alege planul potrivit și folosește butonul de mai jos pentru a trimite dovada plății. Activarea se face manual după verificare.\n\n• 49 lei / lună\n• 279 lei / 6 luni\n• 459 lei / an\n• 1.849 lei permanent (Unlimited)\n\nPlata se face prin Revolut.', color: 0xf59e0b }], components: [{ type: 1, components: [{ type: 2, style: 1, label: '📨 Trimite dovada plății', custom_id: 'panel:billing:proof' }] }] },
    ...
    '🎯・log-acțiuni-organizație': { embeds: [{ title: '🎯 Log acțiuni organizație', description: 'Aici sunt publicate acțiunile înregistrate pentru organizație.', color: 0x3b82f6 }] },
  ...
  try {
    ...
    if (action === 'dashboard_overview' || action === 'repair_guild' || action === 'auto_configure_routes' || action === 'auto_configure_guild' || action === 'set_module_enabled' || action === 'test_setup' || action === 'reset_routes') {
      ...
      if (action === 'reset_routes') {
        ...
        return reply(request, { ok: true, routes: {}, message: 'Rutele canalelor au fost resetate. Embedurile existente nu au fost șterse din Discord.' });
      }
      ...
      if (botMemberResponse?.ok) {
        ...
        for (const role of Array.isArray(roles) ? roles : []) {
          if (botRoleIds.has(String(role.id))) botHighestRolePosition = Math.max(botHighestRolePosition, Number(role.position || 0));
          if (String(role.id) === guildId || botRoleIds.has(String(role.id))) {
            try { permissionValue |= BigInt(String(role.permissions || '0')); } catch (_) {}
          }
        }
      }
      hasAdministrator = (permissionValue & 8n) === 8n;
      botRolePositionOk = botHighestRolePosition > 0 || hasAdministrator;
      ...
      if (action === 'test_setup') {
        ...
        for (const channelId of configuredChannels) {
          ...
          if (response.ok) {
            sent.push(channelId);
            const message = await response.json().catch(() => ({}));
            if (message.id) await new Promise((resolve) => setTimeout(resolve, 4000));
            if (message.id) await fetch(`${DISCORD_API}/channels/${channelId}/messages/${message.id}`, { method: 'DELETE', headers: botHeaders(botToken) }).catch(() => null);
          }
        }
      ...
      return reply(request, { ok: true, repaired: action === 'repair_guild', bot: { online: botOnline, administrator: hasAdministrator, role_position_ok: botRolePositionOk, highest_role_position: botHighestRolePosition, missing_permissions: missingPermissions, permission_status: botMemberResponse ? (missingPermissions.length ? 'missing' : 'ok') : 'unknown', intents_status: botOnline ? 'connected' : 'unknown' }, channels: { total: channelList.length, error: channelError || null }, modules, subscription: { plan: selectedGuild.plan, trial_ends_at: selectedGuild.trial_ends_at || null, premium_ends_at: selectedGuild.premium_ends_at || null, includes: selectedGuild.plan === 'free' ? ['Pontaj', 'Învoiri angajați'] : ['Toate modulele Panel Pro'] }, activity, routes });
    }
