...
const sanitizeCustomModules = (input: any) => {
  ...
  for (const [rawKey, raw] of Object.entries(input)) {
    ...
    const response_flow = value.response_flow && typeof value.response_flow === 'object' ? { enabled: value.response_flow.enabled === true, title: clean(value.response_flow.title || ${label} · Răspuns, 256), log_key: log__response, reason_required: value.response_flow.reason_required === true, notify_submitter: value.response_flow.notify_submitter !== false } : { enabled: false, title: ${label} · Răspuns, log_key: log__response, reason_required: false, notify_submitter: true };\n    const responses = value.responses && typeof value.responses === 'object' ? { success: clean(value.responses.success, 2000), error: clean(value.responses.error, 2000), confirmation: clean(value.responses.confirmation, 2000), visibility: value.responses.visibility === 'public' ? 'public' : 'private' } : { success: '', error: '', confirmation: '', visibility: 'private' };
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action === 'assistant_catalog', 'assistant_schema_check') {
