...
async function provisionOfficialServer(db: any, guildId: string) {
  ...
  const messages: Record<string, any> = {
    ...
    '💎・preturi-si-premium': { embeds: [{ title: '💎 Panel Pro Premium', description: 'Planurile și funcțiile disponibile sunt prezentate pe site-ul oficial: https://bot.panel-pro.ro\n\nPremium poate debloca module și funcții suplimentare în funcție de planul activ. Prețurile afișate pe site sunt cele valabile. Nu efectua plăți prin mesaje private sau linkuri neverificate.', color: 0xf59e0b }] },
  ...
  for (const [name, body] of Object.entries(messages)) { const id=channelIds[name]; if(!id) continue; const existingMessages=await fetch(DISCORD_API + '/channels/' + id + '/messages?limit=50',{headers}).then((r)=>r.ok?r.json():[]).catch(()=>[]); const title=String(body?.embeds?.[0]?.title||''); if(Array.isArray(existingMessages)&&existingMessages.some((m:any)=>(m.embeds||[]).some((e:any)=>String(e.title||'')===title))) continue; const r=await fetch(DISCORD_API + '/channels/' + id + '/messages',{method:'POST',headers,body:JSON.stringify({allowed_mentions:{parse:[]},...body})}); if(!r.ok && r.status!==429) console.error('[provision]',name,r.status); }
