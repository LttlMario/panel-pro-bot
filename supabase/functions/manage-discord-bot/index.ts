...
async function provisionOfficialServer(db: any, guildId: string) {
  ...
  try {
    ...
    if (messages['🧩・module-disponibile']?.embeds?.[0]) messages['🧩・module-disponibile'].embeds[0].description = `Catalog actualizat automat la fiecare configurare (${new Date().toLocaleString('ro-RO')}).\\n\\n${all.map((line)=>'• '+line).join('\\n')}\\n\\nConfigurează și publică modulele din https://bot.panel-pro.ro. Modulele Premium sunt disponibile doar cu abonament activ.`;
