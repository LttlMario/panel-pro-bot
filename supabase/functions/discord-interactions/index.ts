...
const ticketAddMemberModal = (ticketId: string) => ({ type: 9, data: { custom_id: `panel:ticket:add_member_submit:${ticketId}`, title: 'Adaugă membru în ticket', components: [{ type: 1, components: [{ type: 4, custom_id: 'member_id', label: 'ID Discord membru', style: 1, required: true, min_length: 15, max_length: 22, placeholder: 'Ex: 123456789012345678' }] }] } });
const ticketControls = (ticketId: string, closed = false) => [{ type: 1, components: closed ? [{ type: 2, style: 3, label: '🔄 Redeschide ticket', custom_id: `panel:ticket:reopen:${ticketId}` }, { type: 2, style: 2, label: '📄 Vezi transcript', custom_id: `panel:ticket:transcript:${ticketId}` }] : [{ type: 2, style: 3, label: 'Preia ticket', custom_id: `panel:ticket:claim:${ticketId}` }, { type: 2, style: 1, label: '📣 Notifică staff', custom_id: `panel:ticket:notify:${ticketId}` }, { type: 2, style: 2, label: '📄 Transcript', custom_id: `panel:ticket:transcript:${ticketId}` }, { type: 2, style: 1, label: '👥 Adaugă membru', custom_id: `panel:ticket:add_member:${ticketId}` }, { type: 2, style: 4, label: 'Închide ticket', custom_id: `panel:ticket:close:${ticketId}` }] }];
...
const controlPayload = async (db: any, routeKey: string, trialText = '', includeDonation = true, includePremium = true, includeTrial = false) => {
  const definitions: Record<string, { title: string; description: string; color: number; buttons: any[] }> = {
    ...
    weekly_reports: { title: '📊 Raport săptămânal pontaj', description: 'Trimite zilele lucrate, orele pe fiecare membru și totalul săptămânal.', color: 0x06b6d4, buttons: [{ label: 'Generează raport pontaj', style: 1, id: 'panel:discovery:weekly_shift_report' }] },
...
async function handleMarketplaceSubmit(db: any, context: any, kind: 'legal' | 'illegal', values: Record<string, any>) {
  ...
  if (kind === 'illegal') { row.organization_id = context.organization.id; row.subcategorie = String(values.subcategory || '').trim().slice(0, 100) || null; }
...
async function handleStashSubmit(db: any, context: any, kind: 'item' | 'request' | 'donation', values: Record<string, string>) {
  ...
  if (kind === 'item') {
    ...
    const { data, error } = await db.from('discovery_stash_items').insert({ organization_id: context.organization.id, title, category: String(values.category || 'General').trim(), location: String(values.location || 'General').trim(), quantity, unit: 'buc.', description: String(values.description || '').trim(), status: 'available', source_type: 'manual', created_by_discord_id: context.discordId, created_by_name: context.displayName, updated_by_discord_id: context.discordId, created_at: now, updated_at: now }).select('*').single();
    if (error) throw error;
  ...
  if (kind === 'request') {
    ...
    const { data, error } = await db.from('discovery_stash_requests').insert({ organization_id: context.organization.id, item_title: title, location: String(values.location || 'General').trim(), quantity, note: reason, status: 'pending', requested_by_discord_id: context.discordId, requested_by_name: context.displayName, created_at: now, updated_at: now }).select('*').single();
    if (error) throw error;
    try {
  ...
  const { data, error } = await db.from('discovery_stash_donations').insert({ organization_id: context.organization.id, title, category: String(values.category || 'General').trim(), location: String(values.location || 'General').trim(), quantity, unit: 'buc.', note: String(values.note || '').trim(), status: 'pending', donated_by_discord_id: context.discordId, donated_by_name: context.displayName, created_at: now, updated_at: now }).select('*').single();
  if (error) throw error;
  try {
...
async function handleCustomModuleSubmit(db: any, interaction: any, module: any) {
  ...
  const submissionLogKey = `${module.log_key}_submission`;
  const logRoute = context.settings?.discord_channel_routes?.[submissionLogKey]?.[context.target] || context.settings?.discord_channel_routes?.[module.log_key]?.[context.target];
  let logWarning = '';
  if (actions.has('send_log') && logRoute?.channel_id && (module.workflow?.log_events || ['submission']).includes('submission')) {
    ...
    const delivery = await deliverDiscordRoute(db, context.settings, logRoute?.channel_id ? (context.settings?.discord_channel_routes?.[submissionLogKey]?.[context.target] ? submissionLogKey : module.log_key) : module.log_key, JSON.stringify(logPayload), { postOnly: true });
...
async function handleTicket(db: any, interaction: any, customId: string, isButton: boolean, isModalSubmit: boolean) {
  ...
  const isOwner = String(ticket.opened_by_discord_id) === discordId;
  if (action === 'notify') {
    if (!isOwner && !isStaff) return interactionMessage('Doar autorul sau echipa de suport poate notifica staff-ul.');
    ...
    const mention = supportRole?.id ? `<@&${supportRole.id}>` : 'Echipa de suport';
    await ticketDiscordApi(db, `/channels/${ticket.channel_id}/messages`, { method: 'POST', body: JSON.stringify({ content: `📣 ${mention}, <@${discordId}> solicită un răspuns în acest ticket.`, allowed_mentions: supportRole?.id ? { roles: [String(supportRole.id)] } : { parse: [] } }) });
    await ticketEvent(db, ticket, discordId, 'message', { action: 'notify_staff' });
    return interactionMessage('Echipa de suport a fost notificată.');
  }
  if (action === 'transcript') {
    let transcript = String(ticket.transcript || '');
    if (!transcript || ticket.status !== 'closed') { try { const messages = await ticketDiscordApi(db, `/channels/${ticket.channel_id}/messages?limit=100`); transcript = (Array.isArray(messages) ? messages.reverse() : []).map((m: any) => `[${m.timestamp || ''}] ${m.author?.username || m.author?.id || 'utilizator'}: ${String(m.content || '').replace(/\n/g,' ')}`).join('\n'); } catch (_) {} }
    if (!transcript) return interactionMessage('Nu există încă mesaje în transcript.');
    return interactionMessage(`📄 Transcript ticket **${ticket.subject || 'Suport'}**\n\n${transcript.slice(-1850)}`);
  }
  if (action === 'reopen') {
    if (!isStaff) return interactionMessage('Doar echipa de suport poate redeschide ticketul.');
    if (ticket.status !== 'closed') return interactionMessage('Ticketul este deja activ.');
    const { error } = await db.from('discovery_support_tickets').update({ status: 'open', closed_at: null, updated_at: new Date().toISOString() }).eq('id', ticketId).eq('status', 'closed'); if (error) throw error;
    await ticketEvent(db, ticket, discordId, 'message', { action: 'reopened' });
    try { await ticketDiscordApi(db, `/channels/${ticket.channel_id}`, { method: 'PATCH', body: JSON.stringify({ name: `ticket-${String(ticket.opened_by_name || 'suport').toLowerCase().replace(/[^a-z0-9-]+/gi,'-').slice(0,36) || String(ticket.channel_id).slice(-6)}` }) }); } catch (_) {}
    try { if (interaction.message?.id) await ticketDiscordApi(db, `/channels/${ticket.channel_id}/messages/${interaction.message.id}`, { method: 'PATCH', body: JSON.stringify({ components: ticketControls(ticketId) }) }); } catch (_) {}
  ...
  if ((action === 'add_member_submit' && isModalSubmit) || (action === 'add_member_select' && isButton === false)) {
    ...
    await ticketDiscordApi(db, `/channels/${ticket.channel_id}/permissions/${memberId}`, { method: 'PUT', body: JSON.stringify({ id: memberId, type: 1, allow: '68608', deny: '0' }) });
    await ticketEvent(db, ticket, discordId, 'message', { action: 'member_added', member_id: memberId });
    await ticketDiscordApi(db, `/channels/${ticket.channel_id}/messages`, { method: 'POST', body: JSON.stringify({ content: `👥 <@${memberId}> a fost adăugat în ticket de <@${discordId}>.`, allowed_mentions: { users: [memberId] } }) });
    return interactionMessage(`Membrul <@${memberId}> a fost adăugat în ticket.`);
  }
  if (!isOwner && !isStaff) return interactionMessage('Doar autorul sau echipa de suport poate închide ticketul.');
  ...
  try {
    ...
    if (isCustom) {
      ...
      if (customId.startsWith('panel:custom_review:')) {
        ...
        if (error) throw error;
        if (updated) { try { const module = await readCustomModule(db, moduleKey); const context = await resolveCustomModuleContext(db, interaction, module); const events = module.workflow?.log_events || ['submission','approval','rejection','error']; const event = nextStatus === 'approved' ? 'approval' : 'rejection'; const eventLogKey = `${module.log_key}_${event}`; const route = context.settings?.discord_channel_routes?.[eventLogKey]?.[context.target] || context.settings?.discord_channel_routes?.[module.log_key]?.[context.target]; if (route?.channel_id && events.includes(event)) await deliverDiscordRoute(db, context.settings, context.settings?.discord_channel_routes?.[eventLogKey]?.[context.target] ? eventLogKey : module.log_key, JSON.stringify({ allowed_mentions: { parse: [] }, embeds: [{ title: `🧩 ${module.label} · ${nextStatus === 'approved' ? 'aprobat' : 'respins'}`, description: `Solicitarea **${updated.subject}** a fost procesată.`, color: nextStatus === 'approved' ? 0x22c55e : 0xef4444, fields: [{ name: 'Procesat de', value: `<@${context.discordId}>`, inline: true }, { name: 'ID solicitare', value: String(submissionId), inline: true }], timestamp: new Date().toISOString() }] }), { postOnly: true }); const responseFlow = module.response_flow || {}; const responseRoute = context.settings?.discord_channel_routes?.[responseFlow.log_key || `${module.log_key}_response`]?.[context.target]; if (responseFlow.enabled === true && responseRoute?.channel_id) await deliverDiscordRoute(db, context.settings, responseFlow.log_key || `${module.log_key}_response`, JSON.stringify({ allowed_mentions: { parse: [] }, embeds: [{ title: responseFlow.title || `${module.label} · Răspuns`, description: `Solicitarea **${updated.subject}** a primit răspunsul: **${nextStatus === 'approved' ? 'Aprobată' : 'Respinsă'}**.`, color: nextStatus === 'approved' ? 0x22c55e : 0xef4444, fields: [{ name: 'Procesat de', value: `<@${context.discordId}>`, inline: true }, { name: 'ID solicitare', value: String(submissionId), inline: true }], timestamp: new Date().toISOString() }] }), { postOnly: true }); } catch (_) {} }
    ...
    if (isMarketplace && isButton) {
      ...
      if (parts[3] === 'edit' || parts[3] === 'delete' || parts[3] === 'sold') {
        const id = String(parts[4] || '').trim();
        ...
        if (parts[3] === 'sold') {
          const { error: soldError } = await db.from(table).update({ status: 'sold', updated_at: new Date().toISOString() }).eq('id', id).eq('organization_id', context.organization.id).eq('created_by_discord_id', context.discordId);
          if (soldError) throw soldError;
          return reply(interactionMessage('Anunțul a fost marcat ca vândut.'));
        }
    ...
    if (isDiscipline && isModalSubmit) {
      ...
      try {
        const context = await resolveManagementContext(db, interaction, audience, permission as 'write' | 'sanction', audience === 'organization' ? 'organization' : 'departments', audience === 'organization' ? 'discipline_organization' : 'discipline_departments', 'discipline_permissions', `${audience}.${permission}`);
        // Both organization and department discipline flows use the member
        // selected in the Discord picker. Dropping the ID for organization
        // scope caused a lookup for an empty member ID and Discord returned
        // HTTP 404 even when the selected user was the person submitting it.
        result = await handleDisciplineSubmit(db, context, interaction, kind, modalValues(interaction), targetId);
      } catch (error) { console.error('[discord-interactions]', error); result = interactionMessage(readableError(error, 'Înregistrarea disciplinară nu a putut fi salvată.')); }
      const followupId = await sendFollowup(deferred.applicationId, deferred.interactionToken, result);
      if (followupId) { await new Promise((resolve) => setTimeout(resolve, 5000)); await deleteFollowup(deferred.applicationId, deferred.interactionToken, followupId); }
      return new Response(null, { status: 204 });
    }
