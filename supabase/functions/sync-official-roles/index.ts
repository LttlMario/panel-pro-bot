...
Deno.serve(async (request)=>{
  ...
  try{
    ...
    const premium=Boolean(ent&&(!ent.ends_at||Date.parse(String(ent.ends_at))>Date.now()))||Date.parse(String(trial?.value?.ends_at||''))>Date.now();
    const [{data:enabledGuilds},{data:activeEntitlements}]=await Promise.all([
      db.from('discovery_guilds').select('guild_id,organization_id').eq('enabled',true).neq('guild_id',GUILD),
      db.from('discovery_guild_entitlements').select('guild_id,organization_id,ends_at').eq('active',true),
    ]);
    const premiumGuilds=new Set((activeEntitlements||[]).filter((item:any)=>{const end=item.ends_at?Date.parse(String(item.ends_at)):NaN;return Boolean((enabledGuilds||[]).some((guild:any)=>String(guild.guild_id)===String(item.guild_id)&&String(guild.organization_id)===String(item.organization_id)))&&(!Number.isFinite(end)||end>Date.now());}).map((item:any)=>String(item.guild_id)));
    const premiumUsers=new Set<string>();
    for(const premiumGuildId of premiumGuilds){let cursor='';for(let page=0;page<20;page++){const batch=await get(`/guilds/${premiumGuildId}/members?limit=1000${cursor?`&after=${cursor}`:''}`);if(!Array.isArray(batch)||!batch.length)break;for(const member of batch){const uid=String(member?.user?.id||'');if(/^\d{15,22}$/.test(uid))premiumUsers.add(uid);}if(batch.length<1000)break;cursor=String(batch[batch.length-1]?.user?.id||'');if(!cursor)break;}}
    ...
    let assigned=0,removed=0; for(const m of members){const uid=String(m.user?.id||'');if(!/^\d{15,22}$/.test(uid))continue;const current=new Set((m.roles||[]).map(String));if(!current.has(map.member)){const r=await fetch(`${API}/guilds/${GUILD}/members/${uid}/roles/${map.member}`,{method:'PUT',headers});if(r.ok)assigned++;}if(premiumUsers.has(uid)&&!current.has(map.premium)){const r=await fetch(`${API}/guilds/${GUILD}/members/${uid}/roles/${map.premium}`,{method:'PUT',headers});if(r.ok)assigned++;}if(!premiumUsers.has(uid)&&current.has(map.premium)){const r=await fetch(`${API}/guilds/${GUILD}/members/${uid}/roles/${map.premium}`,{method:'DELETE',headers});if(r.ok)removed++;}}
