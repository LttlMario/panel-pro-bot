...
Deno.serve(async (request)=>{
  ...
  try{
    ...
    let assigned=0,removed=0; for(const m of members){const uid=String(m.user?.id||'');if(!/^\d{15,22}$/.test(uid))continue;const current=new Set((m.roles||[]).map(String));if(!current.has(map.member)){const r=await fetch(`${API}/guilds/${GUILD}/members/${uid}/roles/${map.member}`,{method:'PUT',headers});if(r.ok)assigned++;}if(premium&&!current.has(map.premium)){const r=await fetch(`${API}/guilds/${GUILD}/members/${uid}/roles/${map.premium}`,{method:'PUT',headers});if(r.ok)assigned++;}if(!premium&&current.has(map.premium)){const r=await fetch(`${API}/guilds/${GUILD}/members/${uid}/roles/${map.premium}`,{method:'DELETE',headers});if(r.ok)removed++;}}
