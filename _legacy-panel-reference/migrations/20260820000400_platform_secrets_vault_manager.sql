-- Secrete administrate centralizat printr-o funcție Edge protejată.
-- Valorile nu sunt returnate niciodată către browser.

CREATE OR REPLACE FUNCTION public.get_panel_platform_secret(secret_name text)
RETURNS text
LANGUAGE sql
SECURITY DEFINER
...
DECLARE
  ...
  allowed boolean;
BEGIN
  allowed := secret_name IN (
    'project_url',
    'publishable_key',
    'cron_secret',
    'discord_bot_token',
    'platform_owner_discord_ids',
    'status_live_cron_secret',
...
REVOKE ALL ON FUNCTION public.get_panel_platform_secret(text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.get_panel_platform_secret_status() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.set_panel_platform_secret(text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_panel_platform_secret(text) TO service_role;
