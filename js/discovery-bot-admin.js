(() => {
  ...
  const runGlobal = async (action, guildId) => {
    ...
    if (action === 'rename') {
      ...
      if (!window.confirm(`Retragi Premium pentru ${guild.name}? Serverul va reveni imediat la Trial sau Free.`)) return;
      await performGlobal({ action: 'revoke_premium', guild_id: guildId }, 'Premium-ul a fost retras.');
