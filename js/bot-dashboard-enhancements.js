(() => {
  ...
  const hydrate = async (card) => { if (card.dataset.dashboardLoaded) return; const link = card.querySelector('a[href*="configurare-server.html?guild_id="]'); if (!link) return; const match = link.href.match(/[?&]guild_id=([^&]+)/); if (!match) return; card.dataset.guildId = decodeURIComponent(match[1]); card.dataset.dashboardLoaded = 'loading'; const node = document.createElement('section'); node.className = 'server-overview'; node.innerHTML = '<p class="meta">Se verifică statusul botului și configurația…</p>'; const badges = card.querySelector('.badges'); if (badges) badges.after(node); else card.appendChild(node); try { render(card, await call({ action: 'dashboard_overview', guild_id: card.dataset.guildId })); const demoKey = `panel-pro-demo-created-${card.dataset.guildId}`; if (!localStorage.getItem(demoKey)) { await call({ action: 'provision_demo_category', guild_id: card.dataset.guildId }); localStorage.setItem(demoKey, '1'); } card.dataset.dashboardLoaded = 'true'; } catch (error) { node.innerHTML = `<p class="meta" style="color:#fda4af">${esc(error.message)}</p>`; card.dataset.dashboardLoaded = 'error'; } };
