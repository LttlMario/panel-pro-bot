(() => {
  ...
  const render = (card, data) => {
    ...
    wheelButton?.addEventListener('click', async () => {
      wheelButton.disabled = true; wheelButton.textContent = 'Se pornește reminderul…';
      try { const result = await call({ action: 'wheel_reminder_start', guild_id: card.dataset.guildId }); renderWheel(result.reminder); }
      catch (error) { wheelButton.disabled = false; wheelButton.textContent = '🎡 Am dat la roată'; wheelStatus.textContent = error.message || 'Reminderul nu a putut fi pornit.'; }
    });
    node.querySelector('[data-wheel-test]')?.addEventListener('click', async (event) => {
      const button = event.currentTarget;
      const status = node.querySelector('[data-wheel-reminder-status]');
      button.disabled = true; button.textContent = 'Se trimite testul…';
      const showBrowserTest = async () => {
        if (!('Notification' in window)) return false;
        if (Notification.permission === 'default') await Notification.requestPermission();
        if (Notification.permission !== 'granted') return false;
        new Notification('Panel Pro · test notificare', { body: 'Aceasta este notificarea privată de test. Timerul nu a fost modificat.' });
        return true;
      };
      try {
        const browserPrivate = await showBrowserTest();
        const result = await call({ action: 'wheel_reminder_test', guild_id: card.dataset.guildId });
        button.textContent = '✓ Test trimis în privat';
        status.textContent = browserPrivate ? 'Discord + notificare web trimise doar ție.' : 'Discord trimis; notificările web sunt blocate în browser.';
        setTimeout(() => { button.disabled = false; button.textContent = '🔔 Test notificarea'; }, 4000);
      } catch (error) { button.disabled = false; button.textContent = '🔔 Test notificarea'; status.textContent = error.message || 'Testul nu a putut fi trimis.'; }
    });
  };
