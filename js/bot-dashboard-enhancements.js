(() => {
  ...
  const render = (card, data) => {
    ...
    node.querySelector('[data-demo-server]')?.addEventListener('click', async (event) => { const button = event.currentTarget; if (!confirm('Se va crea o categorie cu module demonstrative. Butoanele sunt dezactivate și nu se salvează nimic în baza de date. Continui?')) return; button.disabled = true; button.textContent = 'Se creează demo…'; try { const result = await call({ action: 'provision_demo_category', guild_id: card.dataset.guildId }); button.textContent = `✅ Demo creat · ${result.result?.modules || 0} module`; } catch (error) { button.disabled = false; button.textContent = '🧪 Creează demo module'; node.querySelector('.meta').textContent = error.message; } });
  };
