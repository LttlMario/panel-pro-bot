(() => {
  ...
  insertSyntheticAfter('pontaj', 'weekly_reports');
  ...
  const labels = Object.fromEntries(routeKeys.map((key) => {
    ...
    const fallbackLabels = {
      ...
      weekly_reports: 'Rapoarte săptămânale pontaj',
 ...
 const validChannel = (value) => /^\d{15,22}$/.test(String(value || '').trim());
  ...
  const buildWeeklyReportsPanelPayload = () => ({ allowed_mentions: { parse: [] }, embeds: [{ title: '📊 Rapoarte săptămânale pontaj · Panel Pro', description: 'Generează manual raportul săptămânal cu orele și zilele lucrate. Raportul automat poate fi trimis conform programului configurat.', color: 0x06b6d4, fields: [{ name: 'Manual', value: 'Apasă „Generează raport pontaj” pentru a trimite raportul imediat.', inline: false }, { name: 'Automat', value: 'Raportul poate fi trimis automat săptămânal după configurarea canalului.', inline: false }], footer: { text: 'Panel Pro · Rapoarte pontaj' } }], components: [{ type: 1, components: [{ type: 2, style: 1, label: 'Generează raport pontaj', custom_id: 'panel:discovery:weekly_shift_report' }] }] });
  const syncWeeklyReportsPublishState = (resetStatus = true) => { const button = section.querySelector('#discord-weekly-reports-publish'); const statusNode = section.querySelector('#discord-weekly-reports-publish-status'); if (!button) return; const targets = ['primary', 'secondary'].filter((target) => channelIsAccessible(target, 'weekly_reports')); button.disabled = !targets.length; if (resetStatus && statusNode) statusNode.textContent = targets.length ? `Panoul va fi publicat pe ${targets.length === 1 ? 'canalul configurat' : 'canalele configurate'}.` : 'Selectează canalul pentru rapoarte săptămânale.'; };
  const publishWeeklyReportsPanel = async () => { const button = section.querySelector('#discord-weekly-reports-publish'); const statusNode = section.querySelector('#discord-weekly-reports-publish-status'); const targets = ['primary', 'secondary'].filter((target) => channelIsAccessible(target, 'weekly_reports')); if (!button || !statusNode || !targets.length) return; button.disabled = true; statusNode.textContent = 'Se publică panoul de rapoarte…'; try { const response = await window.sendPanelDiscord('weekly_reports', buildWeeklyReportsPanelPayload(), { messageKey: 'weekly-reports-control', channelRoutes: window.getDiscordChannelRoutes?.() || {} }); const result = await response.clone().json().catch(() => ({})); statusNode.textContent = `Panoul de rapoarte a fost publicat pe ${Number(result.routes || targets.length)} canal${Number(result.routes || targets.length) === 1 ? '' : 'e'}.`; } catch (error) { statusNode.textContent = error.message || 'Panoul de rapoarte nu a putut fi publicat.'; } finally { syncWeeklyReportsPublishState(false); } };  const syncPontajPublishState = (resetStatus = true) => {
  ...
  const render = () => {
    ...
    const weeklyFieldset = grid.querySelector('[data-discord-channel-route="weekly_reports"]')?.closest('fieldset'); if (weeklyFieldset && canPublishDiscordPanels) { weeklyFieldset.insertAdjacentHTML('beforeend', '<div class="mt-3 rounded-lg border border-cyan-900/70 bg-cyan-950/20 p-3"><div class="flex flex-wrap items-center gap-2"><button id="discord-weekly-reports-publish" type="button" disabled class="rounded-xl bg-cyan-500 px-4 py-2 text-xs font-black text-slate-950 disabled:cursor-not-allowed disabled:opacity-40">📌 Publică panoul rapoartelor</button><span id="discord-weekly-reports-publish-status" class="text-xs text-slate-400">Selectează canalul pentru rapoarte.</span></div></div>'); }    grid.querySelectorAll('[data-discord-channel-route]').forEach((select) => { select.onchange = () => { const key = select.dataset.discordChannelRoute; setRoute(key, select.dataset.discordChannelTarget, select.value); syncPontajPublishState(); syncContractsPublishState(); if (['requests_organization', 'requests_departments'].includes(key)) syncRequestPublishState(key); if (['organization', 'departments'].includes(key)) syncAnnouncementPublishState(key); if (key === 'weekly_reports') syncWeeklyReportsPublishState(); }; });
    ...
    section.querySelector('#discord-weekly-reports-publish')?.addEventListener('click', publishWeeklyReportsPanel); syncWeeklyReportsPublishState();
